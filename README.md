# 🎓 AI Coding Independence Course

### From Prompt-and-Pray → Fully Autonomous AI Development

> **For senior/staff iOS & Android developers** who already use AI daily but want to 10× their output with zero hand-holding.

---

```
 💥 Basics  →  ⚡ Power Tools  →  🐝 Agentic  →  🔄 Autonomous
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 GEMINI.md      Custom Agents   Multi-Agent     Ralph Loops
 AGENTS.md      Commands        MCP Servers     PRD → Ship
 Prompting      Hooks           Custom MCP      Full Pipeline
                Subagents       Delegation      Jira Integration
```

---

> ⚠️ **Did you know?** AI creates **1.7× more bugs** than humans. This course teaches you how to fix that — with memory, guardrails, specialized agents, and autonomous loops that make AI actually reliable.

---

## 📱 Demo Project: TaskPulse

All examples use **TaskPulse** — a collaborative task manager app.
- **iOS:** SwiftUI + MVVM + SPM
- **Android:** Compose + MVVM + Gradle

---

## 🗺️ Course Navigation

### [📍 Course Map](00-course-map.md) — Start Here!

### Part 1 — 💥 Foundations (~15 min)
| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 01 | [Why AI Fails](Part%201%20—%20Foundations/01-why-ai-fails.md) | The problem: why AI makes so many mistakes |
| 02 | [GEMINI.md Basics](Part%201%20—%20Foundations/02-gemini-md-basics.md) | Give AI persistent memory about your project |
| 03 | [AGENTS.md](Part%201%20—%20Foundations/03-agents-md.md) | Universal config for all AI tools |
| 04 | [Smart Prompting](Part%201%20—%20Foundations/04-smart-prompting.md) | The CRAC framework + 10 mobile templates |

### Part 2 — ⚡ Power Tools (~12 min)
| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 05 | [Custom Agents](Part%202%20—%20Power%20Tools/05-skills.md) | Auto-triggering reusable instructions |
| 06 | [Commands](Part%202%20—%20Power%20Tools/06-commands.md) | Custom `/slash` commands for your workflow |
| 07 | [Hooks](Part%202%20—%20Power%20Tools/07-hooks.md) | Guardrails that catch what prompts miss |
| 08 | [Subagents](Part%202%20—%20Power%20Tools/08-subagents.md) | Focused workers for parallel tasks |

### Part 3 — 🐝 Agentic Patterns (~12 min)
| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 09 | [Multi-Agent Delegation](Part%203%20—%20Agentic%20Patterns/09-agent-teams.md) | Multiple AI agents working in parallel |
| 10 | [MCP Servers](Part%203%20—%20Agentic%20Patterns/10-mcp-servers.md) | Connect AI to GitHub, Figma, Firebase |
| 11 | [Custom MCP](Part%203%20—%20Agentic%20Patterns/11-custom-mcp.md) | Build your own MCP server |
| 12 | [Ralph Loops](Part%203%20—%20Agentic%20Patterns/12-ralph-loops.md) | Walk away, come back to done |

### Part 4 — 📋 Workflow Methodologies (~10 min)
| # | Guide | What You'll Learn |
|---|-------|-------------------|
| 13 | [PRD-Driven Dev](Part%204%20—%20Workflow%20Methodologies/13-prd-driven-dev.md) | PRD → RFC → Tickets → Code pipeline |
| 14 | [Plan Mode](Part%204%20—%20Workflow%20Methodologies/14-plan-mode-mastery.md) | Think before you code |
| 15 | [Jira Integration](Part%204%20—%20Workflow%20Methodologies/15-jira-integration.md) | Close the ticket ↔ code loop |

### Part 5 — 🎯 Daily Scenarios (~15 min)
| # | Scenario | When To Use |
|---|----------|------------|
| 16 | [Unknown Codebase](Part%205%20—%20Daily%20Scenarios/16-unknown-codebase.md) | Joining a new project |
| 17 | [RFC/SPIKE](Part%205%20—%20Daily%20Scenarios/17-rfc-spike.md) | Technical investigations |
| 18 | [FMEA Analysis](Part%205%20—%20Daily%20Scenarios/18-fmea-analysis.md) | Risk assessment |
| 19 | [Bug Investigation](Part%205%20—%20Daily%20Scenarios/19-bug-investigation.md) | Crash reports & debugging |
| 20 | [Bug Fix](Part%205%20—%20Daily%20Scenarios/20-bug-fix.md) | Safe, targeted fixes |
| 21 | [New Feature](Part%205%20—%20Daily%20Scenarios/21-new-feature.md) | Building from scratch |
| 22 | [Update Feature](Part%205%20—%20Daily%20Scenarios/22-update-feature.md) | Extending existing code |
| 23 | [Feature Flags](Part%205%20—%20Daily%20Scenarios/23-feature-flags.md) | Safe rollouts |
| 24 | [Update Docs](Part%205%20—%20Daily%20Scenarios/24-update-docs.md) | Keeping docs current |
| 25 | [Create PRD](Part%205%20—%20Daily%20Scenarios/25-create-prd.md) | From idea to spec |

### 📎 Appendix
| File | What's Inside |
|------|--------------|
| [A: Cheat Sheet](appendix/A-cheat-sheet.md) | One-page quick reference |
| [B: Prompt Library](appendix/B-prompt-library.md) | 24 copy-paste prompts |
| [C: Resources](appendix/C-resources.md) | Links & further reading |

---

## ⏱️ Course Duration

| Section | Time | Format |
|---------|------|--------|
| Part 1 — Foundations | ~15 min | Live demo + examples |
| Part 2 — Power Tools | ~12 min | Live coding |
| Part 3 — Agentic Patterns | ~12 min | Demo + architecture |
| Part 4 — Methodologies | ~10 min | Workflow walkthrough |
| Part 5 — Scenarios | ~15 min | Quick reference tour |
| **Total** | **~64 min** | |

---

## 🚀 Quick Start

1. **Read** [01-why-ai-fails.md](Part%201%20—%20Foundations/01-why-ai-fails.md) to understand the problem
2. **Create** your first [GEMINI.md](Part%201%20—%20Foundations/02-gemini-md-basics.md)
3. **Level up** with [Custom Agents](Part%202%20—%20Power%20Tools/05-skills.md) and [Hooks](Part%202%20—%20Power%20Tools/07-hooks.md)
4. **Go autonomous** with [Ralph Loops](Part%203%20—%20Agentic%20Patterns/12-ralph-loops.md)
5. **Pick your scenario** from [Part 5](Part%205%20—%20Daily%20Scenarios/) for daily work

---

*Built for mobile devs, by mobile devs. 📱*
